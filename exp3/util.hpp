...
{  
    ...
    {  
        for (auto &x : dizhimiaoshufu[var].registerSet)
        {                  
            jicunqimiaoshufu[x].erase(var); 
        }
...
{
    Siyuanshi siyuanshi = Siyuanshis[var3];
    string x = siyuanshi.left_caozuoshu.val;
    
    string z = siyuanshi.value.val;
    ...
    { 
        ...
        if (siyuanshi.caozuofu.val != "=")
        {
            std::cout << operationMapping[siyuanshi.caozuofu.val] << " " << R << "\n";  
        }
        if (!type_number(x))
        {
...
{  
    Siyuanshi siyuanshi = Siyuanshis[var3];
    string x = siyuanshi.left_caozuoshu.val;
    string y = siyuanshi.caozuoshu_right.val;
    string z = siyuanshi.value.val;
